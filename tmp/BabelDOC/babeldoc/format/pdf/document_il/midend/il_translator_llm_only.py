...
class ILTranslatorLLMOnly:
    ...
    def _build_llm_prompt(
        ...
        # 示例与目标语言一致，避免 zh→en 时固定「英→中」示例误导模型输出中文
        lang_out = (self.translation_config.lang_out or "").strip().lower()
        if lang_out in ("en", "eng", "english"):
            example_block = """### Example (target language = English)
Input:
[
    {"id": 0, "input": "{v1}<style id='2'>你好</style>，世界！", "layout_label": "text"}
]
Output:
[
    {"id": 0, "output": "{v1}<style id='2'>hello</style>, world!"}
]"""
        else:
