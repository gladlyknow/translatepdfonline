...
class PDFCreater:
    ...
    def create_render_units_for_page(
        ...
        # Convert curves to render units (page-level curves + curves from formulas).
        # 始终渲染曲线，保留标题上下线等装饰线；跳过完全落在页面顶端/底端极窄带内的曲线以减杂质
        if not translation_config.skip_curve_render:
            all_curves = list(page.pdf_curve) + formula_curves
            page_crop = page.cropbox.box if page.cropbox and page.cropbox.box else None
            margin_ratio = 0.06
            for i, curve in enumerate(all_curves):
                if getattr(curve, "box", None) and page_crop is not None:
                    try:
                        cmin_y = min(curve.box.y, curve.box.y2)
                        cmax_y = max(curve.box.y, curve.box.y2)
                        ptop = max(page_crop.y, page_crop.y2)
                        pbot = min(page_crop.y, page_crop.y2)
                        h = abs(ptop - pbot)
                        if h > 0:
                            margin = margin_ratio * h
                            if (cmax_y <= pbot + margin and cmin_y >= pbot) or (
                                cmin_y >= ptop - margin and cmax_y <= ptop
                            ):
                                continue
