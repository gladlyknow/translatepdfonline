...
def translate(translation_config: TranslationConfig) -> TranslateResult:
    cancel_event = getattr(translation_config, "cancel_event", None)
    with ProgressMonitor(
        get_translation_stage(translation_config), cancel_event=cancel_event
    ) as pm:
...
def do_translate(
    ...
    try:
        ...
        if result is None:
            logger.error(
                "translate internal error: result is None after pipeline "
                "(merge failed or skipped parts only)"
            )
            raise ExtractTextError(
                "Translation produced no output (merge failed or no translatable parts)."
            )
...
def check_cid_char(il: il_version_1.Document):
    ...
    return cid_count > len(chars) * 1.0
...
def _do_translate_single(
    ...
    if translation_config.only_include_translated_page and not docs.page:
        raise ExtractTextError(
            "No pages match the translation filter in this PDF part "
            "(check page_range and only_include_translated_page; the selected "
            "pages may have no extractable content)."
        )
