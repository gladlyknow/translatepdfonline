"""Options to change pi_heif's runtime behavior."""

DECODE_THREADS = 4
"""Maximum number of threads to use for decoding images(when it is possible)

When use pi_heif as a plugin you can set it with: `register_*_opener(decode_threads=8)`"""


THUMBNAILS = True
"""Option to enable/disable thumbnail support

When use pi_heif as a plugin you can set it with: `register_*_opener(thumbnails=False)`"""


DEPTH_IMAGES = True
"""Option to enable/disable depth image support

When use pi_heif as a plugin you can set it with: `register_*_opener(depth_images=False)`"""


AUX_IMAGES = True
"""Option to enable/disable auxiliary image support

When use pi_heif as a plugin you can set it with: `register_*_opener(aux_images=False)`"""
...
When use pi_heif as a plugin you can set it with: `register_*_opener(quality=-1)`"""


SAVE_HDR_TO_12_BIT = False
"""Should 16 bit images be saved to 12 bit instead of 10 bit``

When use pi_heif as a plugin you can set it with: `register_*_opener(save_to_12bit=True)`"""
...
When use pi_heif as a plugin you can set it with: `register_*_opener(allow_incorrect_headers=True)`"""


SAVE_NCLX_PROFILE = True
"""Should be ``nclx`` profile saved or not.

Default for all previous versions(pi_heif<0.14.0) was NOT TO save `nclx` profile,
...
When use pi_heif as a plugin you can unset it with: `register_*_opener(save_nclx_profile=False)`"""
...
When use pi_heif as a plugin you can set this option with ``preferred_encoder`` key."""
...
When use pi_heif as a plugin you can set this option with ``preferred_decoder`` key."""
