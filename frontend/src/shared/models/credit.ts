...
export enum CreditTransactionScene {
  ...
  TRANSLATE = 'translate', // PDF translate (Route A)
  TRIAL = 'trial', // Creem subscription free trial grant
}
...
export function calculateCreditExpirationTime({
  ...
  // Subscription / billing-period grants: always expire at period end when provided
  if (currentPeriodEnd) {
    return new Date(currentPeriodEnd.getTime());
  }

  // One-time or non-period grants: no fixed period → never expire
