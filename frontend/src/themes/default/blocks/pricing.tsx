...
export function Pricing({
  ...
  const handleCheckout = async (
    ...
    try {
      ...
      const prefix = 'checkout failed:';
      toast.error(
        lower.startsWith(prefix) ? msg : `${prefix} ${msg || 'unknown error'}`
      );
  ...
  // Deep link: /pricing?group=one-time | monthly | quarterly | semi-annual | yearly
  useEffect(() => {
    const g = searchParams.get('group');
    if (!g || !section.groups?.length) return;
    if (section.groups.some((x) => x.name === g)) {
      setGroup(g);
    }
                ...
                <CardHeader>
                  ...
                  {isCurrentPlan ? (
                    ...
                    <Button
                      ...
                      disabled={isLoading}
                      className={cn(
                        'focus-visible:ring-ring inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap focus-visible:ring-1 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50',
