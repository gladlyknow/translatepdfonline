...
export class CreemProvider implements PaymentProvider {
  ...
  constructor(configs: CreemConfigs) {
    this.configs = {
      ...configs,
      // 后台粘贴的 Key 常带首尾空白/换行，会导致 Creem 返回 403
      apiKey: String(configs.apiKey ?? '').trim(),
    };
    this.baseUrl =
  ...
  async createPayment({
    ...
    try {
      if (!this.configs.apiKey) {
        throw new Error('Creem API key is empty; set it in Admin → Payment');
      }
      if (!order.productId) {
      ...
      if (result.error) {
        const e = result.error as { message?: string } | string;
        const msg =
          typeof e === 'string' ? e : (e?.message ?? 'create payment failed');
        throw new Error(msg);
      }

      // create payment success
      const checkoutId = result.id;
      const checkoutUrlRaw = result.checkout_url;
      if (typeof checkoutId !== 'string' || typeof checkoutUrlRaw !== 'string') {
  ...
  async getPaymentSession({
    ...
    try {
      ...
      if (!session.id || !session.order) {
        ...
        const detail =
          typeof err === 'string' ? err : 'get payment failed';
  ...
  async getPaymentEvent({ req }: { req: Request }): Promise<PaymentEvent> {
    try {
      ...
      if (eventType === PaymentEventType.WEBHOOK_NOOP) {
        return {
          eventType,
          eventResult: event,
      ...
      } else if (eventType === PaymentEventType.SUBSCRIPTION_TRIALING) {
        paymentSession = await this.buildPaymentSessionFromSubscription(
          event.object as any
        );
      }
  ...
  async getPaymentBilling({
    ...
    try {
      ...
      const portal = billing.customer_portal_link;
  ...
  private async makeRequest(endpoint: string, method: string, data?: any) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        const parts = [
          ...
          errJson.trace_id ? `trace_id=${errJson.trace_id}` : '',
        ].filter(Boolean);
        if (parts.length) detail = `: ${parts.join(' ')}`;
      } catch {
        if (text && text.length < 500) detail = `: ${text.trim()}`;
      }
      throw new Error(
        `request creem api failed with status: ${response.status}${detail}`
      );
    }
    ...
    try {
      ...
      throw new Error('Invalid JSON from Creem API');
    }
  }
  ...
  private mapCreemEventType(eventType: string): PaymentEventType {
    switch (eventType) {
      ...
      case 'subscription.trialing':
        return PaymentEventType.SUBSCRIPTION_TRIALING;
      ...
      case 'dispute.created':
        return PaymentEventType.WEBHOOK_NOOP;
      default:
        return PaymentEventType.WEBHOOK_NOOP;
    }
  }
  ...
  private mapCreemStatus(session: any): PaymentStatus {
    ...
    // Free trial checkout: no charge yet, subscription in trialing
    if (subscription?.status === 'trialing') {
      return PaymentStatus.SUCCESS;
    }
    const sessionStatus = session.status;
    throw new Error(
      `Unknown Creem session status: ${sessionStatus}, orderStatus: ${orderStatus}`
    );
  }
