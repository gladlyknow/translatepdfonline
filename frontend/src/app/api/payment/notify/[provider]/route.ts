...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      session.subscriptionInfo?.status === SubscriptionStatus.TRIALING
    ) {
      await handleCreemTrialFlow({
        session,
    ...
    if (eventType === PaymentEventType.CHECKOUT_SUCCESS) {
      ...
      if (!existingSubscription) {
        if (session.subscriptionInfo.status === SubscriptionStatus.TRIALING) {
          return Response.json({ message: 'success' });
        }
      ...
      // Trial credits + optional subscription row handled above
    } else {
