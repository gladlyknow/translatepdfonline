...
export async function POST(req: Request) {
  try {
    ...
    if (storageService.getProviderNames().length === 0) {
      return respErr(
        'Storage not configured: set r2_access_key, r2_secret_key, r2_bucket_name (admin or env), and ensure the database can be reached so settings load.'
      );
    }
    ...
    return respErr(
      msg && msg !== 'Unknown error' ? `upload image failed: ${msg}` : 'upload image failed'
    );
  }
}
