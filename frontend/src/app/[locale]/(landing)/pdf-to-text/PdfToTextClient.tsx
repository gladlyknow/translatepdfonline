...
function InsufficientCreditsDialog({
  ...
  const t = useTranslations('pages.pdf-to-text');
  return (
...
export function PdfToTextClient({ children }: { children?: ReactNode }) {
  const t = useTranslations('pages.pdf-to-text');
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      if (translateBilling?.enabled) {
        ...
        const need = 1 * translateBilling.creditsPerPage; // OCR 按文件计费
        if (balance < need) {
