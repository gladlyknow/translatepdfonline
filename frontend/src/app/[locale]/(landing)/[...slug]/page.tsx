...
export async function generateMetadata({
  ...
  // dynamic page slug
  const slugArr = typeof slug === 'string' ? [slug] : (slug as string[]);
  const dynamicPageSlug = slugArr.join('.') || '';

  // Paths like tasks/<taskId>/output-preview-url or tasks/<taskId>/view are API-like, not content pages
  if (
    slugArr[0] === 'tasks' &&
    slugArr.length >= 3 &&
    (slugArr[2] === 'output-preview-url' || slugArr[2] === 'view')
  ) {
    return;
  }
