...
// PDF translate (Route A): documents and translation_tasks
export const documents = table(
  'documents',
  {
    id: text('id').primaryKey(),
    userId: text('user_id').references(() => user.id, { onDelete: 'set null' }),
    anonId: text('anon_id'), // anonymous user scope when not logged in
  ...
  (t) => [
    ...
    index('idx_documents_anon_id').on(t.anonId),
    index('idx_documents_created_at').on(t.createdAt),
  ]
);

export const translationTasks = table(
  'translation_tasks',
  {
    id: text('id').primaryKey(),
    userId: text('user_id').references(() => user.id, { onDelete: 'set null' }),
    ...
    /** 用户填写的页范围原文；仅当与文档取交集后的 page_range 不一致时写入 */
    pageRangeUserInput: text('page_range_user_input'),
    ...
    outputPrimaryPath: text('output_primary_path'),
    outputObjectKey: text('output_object_key'),
    sourceSliceObjectKey: text('source_slice_object_key'),
    preprocessWithOcr: boolean('preprocess_with_ocr').default(false).notNull(),
    /** Shipany credit 表中本次任务对应的 consume 记录 id，幂等扣费 */
    ...
    /** FC 429/503 等下次自动重试时间；null 表示可立即派发 */
    fcNextAttemptAt: timestamp('fc_next_attempt_at'),
    fcLastInvokedAt: timestamp('fc_last_invoked_at'),
    fcLastHttpStatus: integer('fc_last_http_status'),
  ...
  (t) => [
    index('idx_translation_tasks_user_id').on(t.userId),
    index('idx_translation_tasks_anon_id').on(t.anonId),
...
export const docConvertTasks = table(
  ...
  (t) => [
    index('idx_doc_convert_tasks_user_created').on(t.userId, t.createdAt),
    index('idx_doc_convert_tasks_status').on(t.status),
  ]
);
