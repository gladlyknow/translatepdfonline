...
### 二.1 从 GPU 迁到纯 CPU（推荐）

若此前使用 **GPU 函数** 成本过高，可改为 **纯 CPU** 方案：

- **函数类型**：在 FC 控制台创建/重建时选择 **Web 函数**（自定义容器），**不要选 GPU 函数**。
- **镜像**：使用 **CPU 版镜像**（由 `docker/Dockerfile.babeldoc-fc` 构建），即 `.../babeldoc-fc:latest`，**不要使用** `Dockerfile.babeldoc-fc.gpu` 或 `:gpu` tag。
- **规格**：内存 ≥ 2048 MB、超时 ≥ 600 秒、实例并发 1；**不勾选、不配置任何 GPU**。
- ECS 侧无需改代码，仅将 `BABELDOC_FC_URL` 指向新的 CPU 函数 HTTP 触发器地址即可。

快速核对可参考 [FC_CPU_DEPLOY_CHECKLIST.md](FC_CPU_DEPLOY_CHECKLIST.md)。

---

## 三、构建并推送镜像

### 3.0 使用 GitHub Actions 构建（推荐）

项目提供 GitHub Actions 工作流，在推送代码到 `main` 或手动触发时自动构建 **CPU 版** 镜像并推送到阿里云 ACR。

1. **配置 GitHub Secrets**（仓库 Settings → Secrets and variables → Actions，与 GPU 构建共用同一套即可）：
   ...
   - `ALIBABA_CLOUD_PASSWORD`：ACR 登录密码（ACR 控制台设置）。

2. **触发构建**：推送涉及 `babeldoc_fc/`、`docker/Dockerfile.babeldoc-fc`、`tmp/BabelDOC/` 的提交到 `main`，或在 Actions 页选择 “Build BabelDOC FC (CPU) and push to ACR” 后 “Run workflow”。

3. **镜像 tag**：工作流会推送 `latest` 与 `sha-<commit_sha>`，FC 使用 `.../babeldoc-fc:latest` 即可；需回滚时可改为 `.../babeldoc-fc:sha-<commit>`。

若需本地构建或使用 GPU 镜像，请按下面 3.1–3.3 执行。
...
| **超时时间** | 600 秒（10 分钟）；大文件可设为 900–1200 秒 | 单次翻译可能较久，避免中途被 FC 杀死；需 ≥ ECS 的 `BABELDOC_FC_TIMEOUT_SECONDS`。 |
...
| `BABELDOC_FC_SECRET` | 建议 | 与 ECS 配置相同，用于校验请求头 `X-Babeldoc-Secret`（勿用 `X-Fc-*`，FC 会过滤）。 |
...
4. 配置建议：
   ...
   - **认证方式**：可选「匿名」或按需配置「JWT」等（本方案主要靠请求头 `X-Babeldoc-Secret` 鉴权）。
...
请求头需带 **X-Babeldoc-Secret**（与 FC 环境变量 `BABELDOC_FC_SECRET` 一致），否则若 FC 配置了该密钥会返回 403。
...
- [ ] ACR 中已推送镜像（CPU 用 `Dockerfile.babeldoc-fc` / `:latest`，或通过 GitHub Actions 自动推送；GPU 用对应 tag）。
- [ ] FC 函数已创建，运行时为 **自定义容器**，镜像地址正确。
- [ ] **纯 CPU**：函数类型为 **Web 函数**，未选 GPU；内存 ≥ 2048 MB，超时 ≥ 600 秒。
- [ ] **GPU**：已选 GPU 规格；内存与超时同上。
...
| 返回 403 | 未带或错误的 `X-Babeldoc-Secret` | 确认 ECS 与 FC 的 `BABELDOC_FC_SECRET` 一致，且 babeldoc_client 请求头带 `X-Babeldoc-Secret`。 |
