...
def _target_lang_only_instruction(target_lang_norm: str) -> str:
    ...
    if t in ("es", "spa", "spanish"):
        return "CRITICAL: You are translating INTO Spanish only. " + base.replace("TARGET language", "Spanish")
    if t in ("fr", "fra", "fre", "french"):
        return "CRITICAL: You are translating INTO French only. " + base.replace("TARGET language", "French")
    if t in ("it", "ita", "italian"):
        return "CRITICAL: You are translating INTO Italian only. " + base.replace("TARGET language", "Italian")
    if t in ("el", "ell", "gre", "greek"):
        return "CRITICAL: You are translating INTO Greek only. " + base.replace("TARGET language", "Greek")
    if t in ("de", "deu", "ger", "german"):
        return "CRITICAL: You are translating INTO German only. " + base.replace("TARGET language", "German")
    if t in ("ru", "rus", "russian"):
        return "CRITICAL: You are translating INTO Russian only. " + base.replace("TARGET language", "Russian")
...
def run_translate_local(
    ...
    if result is not None:
        ...
        if page_hint is None:
            for seq_attr in ("translated_page_indices", "pages", "translated_pages"):
                seq = getattr(result, seq_attr, None)
                if seq is not None:
                    try:
                        ln = len(seq)
                        if ln >= 1:
