...
def get_fc_secret() -> str:
    """FC 鉴权密钥；与 Next 的 TRANSLATE_FC_SECRET / BABELDOC_FC_SECRET 一致。"""
    return (
        (os.getenv("TRANSLATE_FC_SECRET") or os.getenv("BABELDOC_FC_SECRET") or "")
        .strip()
    )


def get_fc_auth_header() -> str:
    return (os.getenv("TRANSLATE_FC_AUTH_HEADER") or "X-Babeldoc-Secret").strip()


def get_fc_auth_scheme() -> str:
    return os.getenv("TRANSLATE_FC_AUTH_SCHEME") or ""
